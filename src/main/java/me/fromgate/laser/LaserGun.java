    ...
    private void processMineBeam(List<Block> beam) {
        ...
        for (Block b : beam) {
            ...
            if ((b.getType() != Material.AIR) && (this.minelaser)) {
                if (u().rollDiceChance(mine_explode_chance))
                    b.getWorld().createExplosion(b.getLocation().add(0.5, 0.5, 0.5), mine_explode_power, u().rollDiceChance(mine_ignite_chance));
